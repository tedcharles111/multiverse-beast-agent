...
def check_code_errors(project_path: str) -> dict:
    errors = {}
    try:
        result = subprocess.run(['npx', 'eslint', '.', '--ext', '.js,.jsx,.ts,.tsx', '--format', 'json'],
...
def deploy_netlify(project_path: str) -> str:
    ...
    if not token: return "Netlify token not configured."
    dist = os.path.join(project_path, 'dist')
    if not os.path.exists(dist): dist = project_path
    zip_buf = io.BytesIO()
...
def deploy_vercel(project_path: str) -> str:
    ...
    if not token: return "Vercel token not configured."
    dist = os.path.join(project_path, 'dist')
    if not os.path.exists(dist): dist = project_path
...
def deploy_cloudflare(project_path: str) -> str:
    ...
    dist = os.path.join(project_path, 'dist')
    if not os.path.exists(dist): dist = project_path
    # Fall back to wrangler if available, else error
    try:
